...
class SymbolTable:
	...
	class TableNode:
		...
		def getSymbolRegister(self, symbol):
			search_node = self
		...
		def getSymbolLength(self, symbol):
			search_node = self
	...
	def fill_tree(self, AST):
		...
		while len(path) != 0:
			...
			if current_name == "code block":
				...
				if current_ast_node.name != "argument" and current_ast_node.name != "constant":
